...
log_filename = get_absolute_file_path(
    "sql_transformations.log", "logs"
)  # "logs/sql_transformations.log"
...
class SQLAlterations:
    def __init__(self, datastore_config_file: str):
        ...
        self.connection_string = self.connector.create_connection_string(
            datastore_config_file, connect_to_database=True, new_db_name="postgres"
        )
    ...
    def create_database(self, database_name: str):
        ...
        if database_engine:
            ...
            if result.scalar():
                ...
                sql_transformations_logger.warning(
                    f"Database '{database_name}' already exists. Skipping database creation."
                )
            else:
                ...
                sql_transformations_logger.info(
    ...
    def alter_and_update(self, sql_file_path: str):
        ...
        try:
            ...
            sql_transformations_logger.exception(
...
def perform_database_operations(target_datastore_config_file_name):
    ...
    sql_statements.alter_and_update(
        get_absolute_file_path("alter_table_schema.sql", f"sales_data\DDL")
    )  # r'sales_data\DDL\alter_table_schema.sql'
    # Add in the necessary columns
    sql_statements.alter_and_update(
        get_absolute_file_path("column_additions.sql", r"sales_data\DDL")
    )  # r'sales_data\DDL\column_additions.sql'
    ...
    )  # r'sales_data\DML\add_weight_class_column_script.sql')
    # Alter the schema of the dim_product_details table
    sql_statements.alter_and_update(
        get_absolute_file_path(
            "alter_dim_product_details_table_schema.sql", r"sales_data\DDL"
        )
    )  # r'sales_data\DDL\alter_dim_product_details_table_schema.sql')
    ...
    )  # r'sales_data\DDL\orders_table_FK_constraints.sql')
    ...
    )  # r'sales_data\DDL\dim_currency_FK_constraint.sql')
